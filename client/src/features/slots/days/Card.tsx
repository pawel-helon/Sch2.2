...
import { getFilteredSlots } from 'src/utils/data/getFilteredSlots';
...
export const MobileDayCard = (props: {
  year: number,
  weekNumber: number,
  day: string,
...
const MobileDayCardLoaded = (props: {
  slots: Slot[],
  year: number,
  weekNumber: number,
  day: string,
...
export const DesktopDayCard = (props: {
  year: number,
  weekNumber: number,
  day: string,
  isRecurringSlotsOnly: boolean,
}) => {
  const { data, status: slotsStatus } = useSelector((state: RootState) => selectDaySlots(state, props.day));
  const slots = getFilteredSlots(data, props.isRecurringSlotsOnly);

  let content: React.ReactNode;
  if (slotsStatus === 'loading') {
    content = (
      <DesktopDayCardLoading />
    );
  } else {
    content = (
      <DesktopDayCardLoaded
        slots={slots}
        year={props.year}
        weekNumber={props.weekNumber}
        day={props.day}
...
export const DesktopDayCardLoaded = (props: {
  slots: Slot[],
  year: number,
  weekNumber: number,
  day: string,
...
const NoSlots = (props: {
  ...
  const handleAddSlot = () => {
    try {
     addSlot({ employeeId: '06daeca5-1878-4adf-abf4-58045206a555', day: props.day})
    } catch (error) {
      console.error(error);
    }
  }
  ...
  const handleAddRecurringSlot = () =>  {
    try {
     addRecurringSlot({ employeeId: '06daeca5-1878-4adf-abf4-58045206a555', day: props.day})
    } catch (error) {
      console.error(error);
    }
  }
