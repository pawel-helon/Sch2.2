...
describe("updateRecurringSlotHour", () => {
  ...
  test("Returns error if any field is missing.", async () => {
    ...
    await updateRecurringSlotHour(mockRequest as Request, mockResponse as Response);

    expect(mockResponse.send).toHaveBeenCalledWith({
      message: "All fields are required: employeeId, slotId and hour.",
      data: null,
    });
  });
  ...
  test("Returns error if hour is not a number between 0 and 23.", async () => {
    ...
    expect(mockResponse.send).toHaveBeenCalledWith({
      message: "Invalid hour. Expected number between 0 and 23.",
      data: null,
    });
  });
