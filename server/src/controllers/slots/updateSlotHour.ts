...
export const updateSlotHour = async (req: Request, res: Response) => {
  ...
  try {
    validateRequest({ res, endpoint: "updateSlotHour", data: { slotId, hour } });
    ...
    if (!result) return createResponse(res, "Failed to update slot hour.");
    ...
    validateResult({ res, endpoint: "updateSlotHour", data: { prevHour: result.rows[0].prevHour, slot } });
